...
export default function Bubble({
    data, // Comapny relation formed in tree structrue
    changeHandler,
    rows, // All company rows used for search info
    level // Number of levels
}: {
    data: TreeNode;
    changeHandler: (row: Company | undefined) => void;
    rows: Company[],
    level: number
}) {
    console.log(level)
    ...
    useEffect(() => {
        ...
        const hierarchy = d3
            ...
            .sort(
                (
                    ...
                    const aValue = rows.find(
                        (value) => a.data.id === value.company_code
                    )?.annual_revenue;
                    const bValue = rows.find(
        ...
        const node = svg
            ...
            .on("mouseover", function (event: MouseEvent, d) {
                ...
                if (!company) return;
                const text = `Lev: ${company?.level} ${company?.company_name} ${company?.founded_year
                    } ${company?.country} Rev: ${company!.annual_revenue} Emp: ${company!.employees
                    }`;
        ...
        const getLabelText = (
            ...
            const maxChars = Math.floor((d.r * k) / 3);
        ...
        function zoomTo(v: d3.ZoomView) {
            ...
            label.text((d) => getLabelText(d, k));
            label.attr("clip-path", (d) => `circle(${d.r * k})`);
            node.attr(
                "transform",
                (d) => `translate(${(d.x - v[0]) * k},${(d.y - v[1]) * k})`
            );
